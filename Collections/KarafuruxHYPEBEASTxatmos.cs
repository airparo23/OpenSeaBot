...
{
    internal static class KarafuruxHYPEBEASTxatmos
    {
        public static void KarafuruxHYPEBEASTxatmosCollection(
           WebDriver webDriver,
           By Nft,
           By NftToBeClicked,
           ...
           double profit)
        {
            ...
            if (MainPageElementsVariables.isVisible)
            {
                ...
                if (MainPageElementsVariables.isSellButtonVisible)
                {
                    ...
                    var collectionType = new Offer.Offer { Type = Offer.OfferType.KarafuruxHYPEBEASTxatmos };
            ...
            else
            {
                ...
                if (MainPageElementsVariables.bestOfferNumber != MainPageElementsCollections.initialValueKarafuruxHYPEBEASTxatmos) // проверяваме дали best offer-а е по голям от моят последен best offer и ако е - продължавам
                {
                    ...
                    if ((MainPageElementsVariables.bestOfferNumber / MainPageElementsVariables.floorNumber) * 100 < 100 - (fees + profit))
                    {
                        ...
                        MainPageElementsCollections.initialValueKarafuruxHYPEBEASTxatmos = MainPageMethods.CalculateMyOfferNumber(fees, profit + 2, MainPageElementsCollections.initialValueKarafuruxHYPEBEASTxatmos);
                        if (MainPageElementsVariables.isMyOfferOnProfit == true)
                        {
                            var offer = new Offer.Offer { Value = MainPageElementsCollections.initialValueKarafuruxHYPEBEASTxatmos, Type = Offer.OfferType.KarafuruxHYPEBEASTxatmos };
                            ...
                            MainPageMethods.SwapWethForEthIfNeeded(webDriver, NftCollection, MainPageElementsCollections.initialValueKarafuruxHYPEBEASTxatmos);
