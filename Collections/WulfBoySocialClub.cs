...
{
    internal static class WulfBoySocialClub
    {
        public static void WulfBoySocialClubCollection(
           WebDriver webDriver,
           By Nft,
           By NftToBeClicked,
           ...
           double profit)
        {
            ...
            if (MainPageElementsVariables.isVisible)
            {
                ...
                if (MainPageElementsVariables.isSellButtonVisible)
                {
                    ...
                    var collectionType = new Offer.Offer { Type = Offer.OfferType.WulfBoySocialClub };
            ...
            else
            {
                ...
                if (MainPageElementsVariables.bestOfferNumber != MainPageElementsCollections.initialValuewulfBoySocialClub) // проверяваме дали best offer-а е по голям от моят последен best offer и ако е - продължавам
                {
                    ...
                    if ((MainPageElementsVariables.bestOfferNumber / MainPageElementsVariables.floorNumber) * 100 < 100 - (fees + profit))
                    {
                        ...
                        MainPageElementsCollections.initialValuewulfBoySocialClub = MainPageMethods.CalculateMyOfferNumber(fees, profit + 2, MainPageElementsCollections.initialValuewulfBoySocialClub);
                        if (MainPageElementsVariables.isMyOfferOnProfit == true)
                        {
                            var offer = new Offer.Offer { Value = MainPageElementsCollections.initialValuewulfBoySocialClub, Type = Offer.OfferType.WulfBoySocialClub };
                            ...
                            MainPageMethods.SwapWethForEthIfNeeded(webDriver, NftCollection, MainPageElementsCollections.initialValuewulfBoySocialClub);
