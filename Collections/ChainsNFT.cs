...
{
    internal static class ChainsNFT
    {
        public static void ChainsNFTCollection(
           WebDriver webDriver,
           By Nft,
           By NftToBeClicked,
           ...
           double profit)
        {
            ...
            if (MainPageElementsVariables.isVisible)
            {
                ...
                if (MainPageElementsVariables.isSellButtonVisible)
                {
                    ...
                    var collectionType = new Offer.Offer { Type = Offer.OfferType.ChainsNFT };
            ...
            else
            {
                ...
                if (MainPageElementsVariables.bestOfferNumber != MainPageElementsCollections.initialValueChainsNFT) // проверяваме дали best offer-а е по голям от моят последен best offer и ако е - продължавам
                {
                    ...
                    if ((MainPageElementsVariables.bestOfferNumber / MainPageElementsVariables.floorNumber) * 100 < 100 - (fees + profit))
                    {
                        ...
                        MainPageElementsCollections.initialValueChainsNFT = MainPageMethods.CalculateMyOfferNumber(fees, profit + 2, MainPageElementsCollections.initialValueChainsNFT);
                        if (MainPageElementsVariables.isMyOfferOnProfit == true)
                        {
                            var offer = new Offer.Offer { Value = MainPageElementsCollections.initialValueChainsNFT, Type = Offer.OfferType.ChainsNFT };
                            ...
                            MainPageMethods.SwapWethForEthIfNeeded(webDriver, NftCollection, MainPageElementsCollections.initialValueChainsNFT);
