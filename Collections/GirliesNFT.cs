...
{
    internal static class GirliesNFT
    {
        public static void GirliesNFTCollection(
           WebDriver webDriver,
           By Nft,
           By NftToBeClicked,
           ...
           double profit)
        {
            ...
            if (MainPageElementsVariables.isVisible)
            {
                ...
                if (MainPageElementsVariables.isSellButtonVisible)
                {
                    ...
                    var collectionType = new Offer.Offer { Type = Offer.OfferType.GirliesNFT };
            ...
            {
                if(MainPageElementsCollections.isGirliesNFTProfitable == true)
                {
                    ...
                    if (MainPageElementsVariables.bestOfferNumber > MainPageElementsCollections.initialValueGirliesNFT) // проверяваме дали best offer-а е по голям от моят последен best offer и ако е - продължавам
                    {
                        ...
                        MainPageElementsVariables.maxAvgPrice > MainPageElementsVariables.floorNumber)
                        {
                            ...
                            MainPageElementsCollections.initialValueGirliesNFT = MainPageMethods.CalculateMyOfferNumber(fees, profit + 2, MainPageElementsCollections.initialValueGirliesNFT);
                            if (MainPageElementsVariables.isMyOfferOnProfit == true)
                            {
                                var offer = new Offer.Offer { Value = MainPageElementsCollections.initialValueGirliesNFT, Type = Offer.OfferType.GirliesNFT };
                                ...
                                MainPageMethods.SwapWethForEthIfNeeded(webDriver, NftCollection, MainPageElementsCollections.initialValueGirliesNFT);
                        ...
                        {
                            MainPageElementsCollections.isGirliesNFTProfitable = false;
                        }
                    }
                
                } else
                {
                    MainPageElementsCollections.isGirliesNFTProfitableCounter++;
                    if(MainPageElementsCollections.isGirliesNFTProfitableCounter >= 300)
                    {
                        MainPageElementsCollections.isGirliesNFTProfitable = true;
                        MainPageElementsCollections.isGirliesNFTProfitableCounter = 0;
                    }
                }
            }
        }
