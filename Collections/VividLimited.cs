...
namespace OpenSeaBot.Collections
{
    ...
    {
        public static void VividLimitedCollection(
           WebDriver webDriver,
           By Nft,
           By NftToBeClicked,
           ...
           double profit)
        {
            ...
            {
                if(MainPageElementsCollections.isVividLimitedProfitable == true)
                {
                    ...
                    if (MainPageElementsVariables.bestOfferNumber > MainPageElementsCollections.initialValueOfferVividLimited) // проверяваме дали best offer-а е по голям от моят последен best offer и ако е - продължавам
                    {
                        ...
                        MainPageElementsVariables.maxAvgPrice > MainPageElementsVariables.floorNumber)
                        {
                            ...
                            MainPageElementsCollections.initialValueOfferVividLimited = MainPageMethods.CalculateMyOfferNumber(fees, profit + 2, MainPageElementsCollections.initialValueOfferVividLimited);
                            if (MainPageElementsVariables.isMyOfferOnProfit == true)
                            {
                                var offer = new Offer.Offer { Value = MainPageElementsCollections.initialValueOfferVividLimited, Type = Offer.OfferType.VividLimited };
                                ...
                                MainPageMethods.SwapWethForEthIfNeeded(webDriver, NftCollection, MainPageElementsCollections.initialValueOfferVividLimited);
                        ...
                        {
                            MainPageElementsCollections.isVividLimitedProfitable = false;
                        }
                    }
                
                } else
                {
                    MainPageElementsCollections.isVividLimitedProfitableCounter++;
                    if(MainPageElementsCollections.isVividLimitedProfitableCounter >= 300)
                    {
                        MainPageElementsCollections.isVividLimitedProfitable = true;
                        MainPageElementsCollections.isVividLimitedProfitableCounter = 0;
                    }
                }
            }
        }
