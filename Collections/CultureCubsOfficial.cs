...
{
    internal static class CultureCubsOfficial
    {
        public static void CultureCubsOfficialCollection(
           WebDriver webDriver,
           By Nft,
           By NftToBeClicked,
           ...
           double profit)
        {
            ...
            if (MainPageElementsVariables.isVisible)
            {
                ...
                if (MainPageElementsVariables.isSellButtonVisible)
                {
                    ...
                    var collectionType = new Offer.Offer { Type = Offer.OfferType.CultureCubsOfficial };
            ...
            else
            {
                ...
                if (MainPageElementsVariables.bestOfferNumber != MainPageElementsCollections.initialValueCultureCubsOfficial) // проверяваме дали best offer-а е по голям от моят последен best offer и ако е - продължавам
                {
                    ...
                    if ((MainPageElementsVariables.bestOfferNumber / MainPageElementsVariables.floorNumber) * 100 < 100 - (fees + profit))
                    {
                        ...
                        MainPageElementsCollections.initialValueCultureCubsOfficial = MainPageMethods.CalculateMyOfferNumber(fees, profit + 2, MainPageElementsCollections.initialValueCultureCubsOfficial);
                        if (MainPageElementsVariables.isMyOfferOnProfit == true)
                        {
                            var offer = new Offer.Offer { Value = MainPageElementsCollections.initialValueCultureCubsOfficial, Type = Offer.OfferType.CultureCubsOfficial };
                            ...
                            MainPageMethods.SwapWethForEthIfNeeded(webDriver, NftCollection, MainPageElementsCollections.initialValueCultureCubsOfficial);
