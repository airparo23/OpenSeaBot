...
{
    internal static class TheMekabots
    {
        public static void TheMekabotsCollection(
           WebDriver webDriver,
           By Nft,
           By NftToBeClicked,
           ...
           double profit)
        {
            ...
            if (MainPageElementsVariables.isVisible)
            {
                ...
                if (MainPageElementsVariables.isSellButtonVisible)
                {
                    ...
                    var collectionType = new Offer.Offer { Type = Offer.OfferType.TheMekabots };

                    MainPageMethods.CalculateMySellNumber(fees, profit, collectionType, webDriver);
                    MainPageMethods.GoToCollection(webDriver, MainPageElements.myAccountUrl);
                    MainPageMethods.GoIntoNft(webDriver, NftToBeClicked);
                    MainPageMethods.ClickSellButton(webDriver);
                    MainPageMethods.TypeMySellNumberAndCompleteListing(webDriver); // here
            ...
            {
                if(MainPageElementsCollections.isTheMekabotsProfitable == true)
                {
                    ...
                    if (MainPageElementsVariables.bestOfferNumber > MainPageElementsCollections.initialValuetheMekabots) // проверяваме дали best offer-а е по голям от моят последен best offer и ако е - продължавам
                    {
                        ...
                        MainPageElementsVariables.maxAvgPrice > MainPageElementsVariables.floorNumber)
                        {
                            ...
                            MainPageElementsCollections.initialValuetheMekabots = MainPageMethods.CalculateMyOfferNumber(fees, profit + 2, MainPageElementsCollections.initialValuetheMekabots);
                            if (MainPageElementsVariables.isMyOfferOnProfit == true)
                            {
                                var offer = new Offer.Offer { Value = MainPageElementsCollections.initialValuetheMekabots, Type = Offer.OfferType.TheMekabots };
                                ...
                                MainPageMethods.SwapWethForEthIfNeeded(webDriver, NftCollection, MainPageElementsCollections.initialValuetheMekabots);
                        ...
                        {
                            MainPageElementsCollections.isTheMekabotsProfitable = false;
                        }
                    }
                
                } else
                {
                    MainPageElementsCollections.isTheMekabotsProfitableCounter++;
                    if(MainPageElementsCollections.isTheMekabotsProfitableCounter >= 300)
                    {
                        MainPageElementsCollections.isTheMekabotsProfitable = true;
                        MainPageElementsCollections.isTheMekabotsProfitableCounter = 0;
                    }
                }
            }
        }
